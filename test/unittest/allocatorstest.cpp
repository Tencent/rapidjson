...
void TestAllocator(Allocator& a) {
    EXPECT_TRUE(a.Malloc(0) == RAPIDJSON_NULLPTR);

    uint8_t* p = static_cast<uint8_t*>(a.Malloc(100));
    EXPECT_TRUE(p != RAPIDJSON_NULLPTR);
    for (size_t i = 0; i < 100; i++)
        p[i] = static_cast<uint8_t>(i);

    // Expand
    uint8_t* q = static_cast<uint8_t*>(a.Realloc(p, 100, 200));
    EXPECT_TRUE(q != RAPIDJSON_NULLPTR);
    for (size_t i = 0; i < 100; i++)
        EXPECT_EQ(i, q[i]);
    ...
    EXPECT_TRUE(r != RAPIDJSON_NULLPTR);
    ...
    EXPECT_TRUE(a.Realloc(a.Malloc(1), 1, 0) == RAPIDJSON_NULLPTR);
}
...
void TestStdAllocator(const Allocator& a) {
    ...
    EXPECT_TRUE(arr != RAPIDJSON_NULLPTR);
    for (int i = 0; i < 10; ++i) {
        arr[i] = 0x0f0f0f0f;
    }
    ...
    for (int i = 1; i < 10; i++) {
        ...
        EXPECT_TRUE(d != RAPIDJSON_NULLPTR);
...
TEST(Allocator, MemoryPoolAllocator) {
    ...
    for (size_t i = 1; i < 1000; i++) {
        EXPECT_TRUE(a.Malloc(i) != RAPIDJSON_NULLPTR);
