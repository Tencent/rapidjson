...
TEST ( Document, Parse_Encoding )
{
  ...
  doc.Parse<kParseDefaultFlags, UTF8<>>
                                     ( s2.c_str() ); // VS2010 or below cannot handle templated function overloading. Use const char* instead.
#else
...
TEST ( Document, AcceptWriter )
{
  ...
  EXPECT_EQ ( "{\"hello\":\"world\",\"t\":true,\"f\":false,\"n\":null,\"i\":123,\"pi\":3.1416,\"a\":[1,2,3,4]}",
              os.str() );
}
...
TEST ( Document, UTF16_Document )
{
  ...
  EXPECT_EQ ( 0, memcmp ( L"Wed Oct 30 17:13:20 +0000 2012", s.GetString(),
                          ( s.GetStringLength() + 1 ) * sizeof ( wchar_t ) ) );
}
