...
TEST ( Value, EqualtoOperator )
{
  ...
  .AddMember ( "a", Value ( kArrayType ).Move().PushBack ( 1, allocator ).PushBack ( 2, allocator ).PushBack ( 3,
               allocator ), allocator );
...
TEST ( Value, IsLosslessDouble )
{
  ...
  EXPECT_FALSE ( Value ( static_cast<int64_t> ( RAPIDJSON_UINT64_C2 ( 0x7FFFFFFF,
                         0xFFFFFFFF ) ) ).IsLosslessDouble() ); // INT64_MAX
  EXPECT_FALSE ( Value ( -static_cast<int64_t> ( RAPIDJSON_UINT64_C2 ( 0x7FFFFFFF,
                         0xFFFFFFFF ) ) ).IsLosslessDouble() ); // -INT64_MAX
  EXPECT_TRUE ( Value ( -static_cast<int64_t> ( RAPIDJSON_UINT64_C2 ( 0x7FFFFFFF,
                        0xFFFFFFFF ) ) - 1 ).IsLosslessDouble() ); // INT64_MIN
...
static void TestArray ( T& x, Allocator& allocator )
{
  ...
  // PushBack(GenericValue&&, Allocator&);
  {
    ...
    y2.PushBack ( std::move ( Value ( kArrayType ).PushBack ( Value ( 1 ), allocator ).PushBack ( "foo", allocator ) ),
                  allocator );
