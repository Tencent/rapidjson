...
TEST ( SchemaValidator, Ref_AllOf )
{
  ...
  INVALIDATE ( s,
               ...
               "/properties/shipping_address", "allOf", "/shipping_address",
               "{ \"required\": {"
...
TEST ( SchemaValidator, Object_Properties )
{
  ...
  INVALIDATE ( s, "{ \"number\": \"1600\", \"street_name\": \"Pennsylvania\", \"street_type\": \"Avenue\" }",
...
TEST ( SchemaValidator, Object_AdditionalPropertiesObject )
{
  ...
  INVALIDATE ( s,
               ...
               "/additionalProperties", "type", "/office_number",
               "{ \"type\": {"
...
private:
  typedef GenericDocument<typename SchemaDocumentType::EncodingType, MemoryPoolAllocator<>, MemoryPoolAllocator<>>
      DocumentType;
...
TEST ( SchemaValidator, TestSuite )
{
  ...
  for ( size_t i = 0; i < sizeof ( filenames ) / sizeof ( filenames[0] ); i++ )
  {
    ...
    {
      GenericDocument<UTF8<>, MemoryPoolAllocator<>, MemoryPoolAllocator<>> d ( &documentAllocator, 1024,
      ...
      else
      {
        ...
        {
          {
            SchemaDocumentType schema ( ( *schemaItr ) ["schema"], filenames[i], static_cast<SizeType> ( strlen ( filenames[i] ) ),
                                        &provider, &schemaAllocator );
            GenericSchemaValidator<SchemaDocumentType, BaseReaderHandler<UTF8<>>, MemoryPoolAllocator<>> validator ( schema,
                &validatorAllocator );
...
TEST ( SchemaValidator, Ref_remote )
{
  ...
  typedef GenericSchemaValidator<SchemaDocumentType, BaseReaderHandler<UTF8<>>, MemoryPoolAllocator<>>
      SchemaValidatorType;
...
TEST ( SchemaValidator, Ref_remote_issue1210 )
{
  ...
  public:
    ...
    virtual const SchemaDocument* GetRemoteDocument ( const char* uri, SizeType length )
    {
      ...
      while ( collection[i] && SchemaDocument::URIType ( uri, length ) != collection[i]->GetURI() )
      {
        ++i;
      }
