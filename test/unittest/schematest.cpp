...
{\
    ...
    if (expected) {\
      ...
      EXPECT_TRUE(validator.GetInvalidSchemaKeyword() == RAPIDJSON_NULLPTR);\
    }\
...
{\
    ...
    ASSERT_TRUE(validator.GetInvalidSchemaKeyword() != RAPIDJSON_NULLPTR);\
...
TEST(SchemaValidator, SchemaPointer) {
    ...
    sd.Parse(
        ...
        "  }"
        "}");
    SchemaDocument s1(sd, RAPIDJSON_NULLPTR, 0, RAPIDJSON_NULLPTR, RAPIDJSON_NULLPTR, Pointer("#/paths/~1some~1path/post/parameters/0/schema"));
    VALIDATE(s1,
        "{"
        "  \"a\": {"
        "    \"c\": \"C1\","
    ...
    INVALIDATE(s1,
        ...
        "}}");
    SchemaDocument s2(sd, RAPIDJSON_NULLPTR, 0, RAPIDJSON_NULLPTR, RAPIDJSON_NULLPTR, Pointer("#/paths/~1some~1path/post/responses/200/schema"));
    VALIDATE(s2,
        ...
        schemaAllocator_(schemaBuffer_, sizeof(schemaBuffer_)) 
    {
        ...
        for (size_t i = 0; i < kCount; i++) {
            ...
            if (!json) {
                ...
                d.Parse(json);
                sd_[i] = new SchemaDocumentType(d, uris[i], static_cast<SizeType>(strlen(uris[i])), RAPIDJSON_NULLPTR, &schemaAllocator_);
...
TEST(SchemaValidator, TestSuite) {
    ...
    const char* onlyRunDescription = RAPIDJSON_NULLPTR;
...
TEST(SchemaValidator, Ref_remote) {
    ...
    SchemaDocumentType s(sd, RAPIDJSON_NULLPTR, 0, &provider);
...
TEST(SchemaValidator, Ref_internal_id_and_schema_pointer) {
    ...
    typedef GenericPointer<Value, MemoryPoolAllocator<> > PointerType;
    SchemaDocumentType s(sd, RAPIDJSON_NULLPTR, 0, RAPIDJSON_NULLPTR, RAPIDJSON_NULLPTR, PointerType("/schema"));
    typedef GenericSchemaValidator<SchemaDocumentType, BaseReaderHandler<UTF8<> >, MemoryPoolAllocator<> > SchemaValidatorType;
...
TEST(SchemaValidator, Ref_remote_issue1210) {
    class SchemaDocumentProvider : public IRemoteSchemaDocumentProvider {
        ...
        SchemaDocumentProvider(const SchemaDocumentProvider&) : collection(RAPIDJSON_NULLPTR) {
        }
    ...
    SchemaDocument* collection[] = { RAPIDJSON_NULLPTR, RAPIDJSON_NULLPTR, RAPIDJSON_NULLPTR };
