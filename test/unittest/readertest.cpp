...
struct ParseBoolHandler : BaseReaderHandler<UTF8<>, ParseBoolHandler<expect>>
{
  ...
  bool Bool ( bool b )
  {
    /*EXPECT_EQ(expect, b); */EXPECT_TRUE ( expect == b );
    ++step_;
    return true;
  }
...
#define TEST_INTEGER(Handler, str, x) \
  ...
  TEST_INTEGER ( ParseUint64Handler, "4294967296", RAPIDJSON_UINT64_C2 ( 1,
                 0 ) ); // 2^32 (max of unsigned + 1, force to use uint64_t)
  TEST_INTEGER ( ParseUint64Handler, "18446744073709551615", RAPIDJSON_UINT64_C2 ( 0xFFFFFFFF,
                 0xFFFFFFFF ) ); // 2^64 - 1 (max of uint64_t)
  ...
  TEST_INTEGER ( ParseInt64Handler, "-9223372036854775808", static_cast<int64_t> ( RAPIDJSON_UINT64_C2 ( 0x80000000,
                 0x00000000 ) ) );    // -2^63 (min of int64_t)
...
struct ParseStringHandler : BaseReaderHandler<Encoding, ParseStringHandler<Encoding>>
{
  ...
  ~ParseStringHandler()
  {
    ...
    if ( copy_ )
    {
      free ( const_cast<typename Encoding::Ch*> ( str_ ) );
    }
  }
...
#define TEST_STRINGARRAY2(Encoding, utype, earray, xarray) \
  ...
  TEST_STRINGARRAY2 ( UTF32<>, unsigned, ARRAY ( 'H', 'e', 'l', 'l', 'o', '\0' ), ARRAY ( '\"', 'H', 'e', 'l', 'l', 'o',
                      '\"', '\0' ) );
  TEST_STRINGARRAY2 ( UTF32<>, unsigned, ARRAY ( 'H', 'e', 'l', 'l', 'o', '\n', 'W', 'o', 'r', 'l', 'd', '\0' ),
                      ARRAY ( '\"', 'H', 'e', 'l', 'l', 'o', '\\', 'n', 'W', 'o', 'r', 'l', 'd', '\"', '\0' ) );
  TEST_STRINGARRAY2 ( UTF32<>, unsigned, ARRAY ( '\"', '\\', '/', '\b', '\f', '\n', '\r', '\t', '\0' ), ARRAY ( '\"',
                      '\\', '\"', '\\', '\\', '/', '\\', 'b', '\\', 'f', '\\', 'n', '\\', 'r', '\\', 't', '\"', '\0' ) );
  TEST_STRINGARRAY2 ( UTF32<>, unsigned, ARRAY ( 0x00024, 0x0000 ), ARRAY ( '\"', '\\', 'u', '0', '0', '2', '4', '\"',
                      '\0' ) );
  TEST_STRINGARRAY2 ( UTF32<>, unsigned, ARRAY ( 0x000A2, 0x0000 ), ARRAY ( '\"', '\\', 'u', '0', '0', 'A', '2', '\"',
                      '\0' ) ); // Cents sign U+00A2
  TEST_STRINGARRAY2 ( UTF32<>, unsigned, ARRAY ( 0x020AC, 0x0000 ), ARRAY ( '\"', '\\', 'u', '2', '0', 'A', 'C', '\"',
                      '\0' ) ); // Euro sign U+20AC
  TEST_STRINGARRAY2 ( UTF32<>, unsigned, ARRAY ( 0x1D11E, 0x0000 ), ARRAY ( '\"', '\\', 'u', 'D', '8', '3', '4', '\\',
                      'u', 'D', 'D', '1', 'E', '\"', '\0' ) );  // G clef sign U+1D11E
...
#define TEST_STRINGENCODING_ERROR(Encoding, TargetEncoding, utype, array) \
  ...
  TEST_STRINGENCODING_ERROR ( UTF8<>, UTF16<>, unsigned char, ARRAY ( '[', '\"', 0xF0u, 0x80u, 0x80u, 0xAFu, '\"', ']',
                              '\0' ) );
  ...
  TEST_STRINGENCODING_ERROR ( UTF8<>, UTF16<>, unsigned char, ARRAY ( '[', '\"', 0xF0u, 0x8Fu, 0xBFu, 0xBFu, '\"', ']',
                              '\0' ) );
  ...
  TEST_STRINGENCODING_ERROR ( UTF8<>, UTF16<>, unsigned char, ARRAY ( '[', '\"', 0xF0u, 0x80u, 0x80u, 0x80u, '\"', ']',
                              '\0' ) );
