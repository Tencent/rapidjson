...
struct GenericStringRef {
    ...
    GenericStringRef(const CharType* str, SizeType len)
        : s(RAPIDJSON_LIKELY(str) ? str : emptyString), length(len) { RAPIDJSON_ASSERT(str != NULL || len == 0u); }
...
private:
    ...
    struct ShortString {
        enum { MaxChars = sizeof(static_cast<Flag*>(NULL)->payload) / sizeof(Ch), MaxSize = MaxChars - 1, LenPos = MaxSize };
        Ch str[MaxChars];
    ...
    */
    ...
    GenericDocument& ParseStream(InputStream& is) {
        GenericReader<SourceEncoding, Encoding, StackAllocator> reader(
            stack_.HasAllocator() ? &stack_.GetAllocator() : NULL);
        ClearStackOnExit scope(*this);
