...
public:
    ...
    defaultValueLength_ ( 0 )
  {
    ...
    if ( const ValueType* v = GetMember ( value, GetAdditionalPropertiesString() ) )
    {
      ...
      {
        schemaDocument->CreateSchema ( &additionalPropertiesSchema_, p.Append ( GetAdditionalPropertiesString(), allocator_ ),
                                       *v, document );
      }
    }
            ...
            if ( const GenericSchemaDocument* remoteDocument = remoteProvider_->GetRemoteDocument ( s, i ) )
            {
              ...
              if ( pointer.IsValid() )
              {
                if ( const SchemaType* sc = remoteDocument->GetSchema ( pointer ) )
                {
                  ...
                  new ( schemaMap_.template Push<SchemaEntry>() ) SchemaEntry ( source, const_cast<SchemaType*> ( sc ), false,
                      allocator_ );
                  return true;
                }
              }
