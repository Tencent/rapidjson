...
public:
    ...
    Hasher(Allocator* allocator = RAPIDJSON_NULLPTR, size_t stackCapacity = kDefaultSize) : stack_(allocator, stackCapacity) {}
...
private:
    ...
    bool WriteType(Type type) { return WriteBuffer(type, RAPIDJSON_NULLPTR, 0); }
...
private:
    ...
    static const ValueType* GetMember(const ValueType& value, const ValueType& name) {
        ...
        return itr != value.MemberEnd() ? &(itr->value) : RAPIDJSON_NULLPTR;
    }
    ...
    bool CreateParallelValidator(Context& context) const {
        ...
        if (validatorCount_) {
            RAPIDJSON_ASSERT(context.validators == RAPIDJSON_NULLPTR);
...
public:
    ...
    */
    explicit GenericSchemaDocument(const ValueType& document, const Ch* uri = RAPIDJSON_NULLPTR, SizeType uriLength = 0,
        IRemoteSchemaDocumentProviderType* remoteProvider = RAPIDJSON_NULLPTR, Allocator* allocator = RAPIDJSON_NULLPTR,
        ...
        schemaRef_(allocator, kInitialSchemaRefSize)
    {
        ...
        RAPIDJSON_ASSERT(root_ != RAPIDJSON_NULLPTR);
    ...
    GenericSchemaDocument(GenericSchemaDocument&& rhs) RAPIDJSON_NOEXCEPT :
        ...
        docId_(rhs.docId_)
    {
        rhs.remoteProvider_ = RAPIDJSON_NULLPTR;
        rhs.allocator_ = RAPIDJSON_NULLPTR;
        rhs.ownAllocator_ = RAPIDJSON_NULLPTR;
        rhs.typeless_ = RAPIDJSON_NULLPTR;
    }
#endif
...
private:
    ...
    void CreateSchemaRecursive(const SchemaType** schema, const PointerType& pointer, const ValueType& v, const ValueType& document, const UriType& id) {
        if (v.GetType() == kObjectType) {
            ...
            for (typename ValueType::ConstMemberIterator itr = v.MemberBegin(); itr != v.MemberEnd(); ++itr)
                CreateSchemaRecursive(RAPIDJSON_NULLPTR, pointer.Append(itr->name, allocator_), itr->value, document, newid);
        }
        else if (v.GetType() == kArrayType)
            for (SizeType i = 0; i < v.Size(); i++)
                CreateSchemaRecursive(RAPIDJSON_NULLPTR, pointer.Append(i, allocator_), v[i], document, id);
    }
    ...
    ValueType* FindId(const ValueType& doc, const UriType& finduri, PointerType& resptr, const UriType& baseuri, bool full, const PointerType& here = PointerType()) const {
        SizeType i = 0;
        ValueType* resval = RAPIDJSON_NULLPTR;
...
public:
    ...
    */
    GenericSchemaValidator(
        ...
        StateAllocator* allocator = RAPIDJSON_NULLPTR, 
        ...
        ownStateAllocator_(RAPIDJSON_NULLPTR),
        schemaStack_(allocator, schemaStackCapacity),
        documentStack_(allocator, documentStackCapacity),
        outputHandler_(RAPIDJSON_NULLPTR),
        error_(kObjectType),
        currentError_(),
        missingDependents_(),
        valid_(true),
    ...
    */
    ...
    void AboveMaximum(int64_t actual, const SValue& expected, bool exclusive) {
        AddNumberError(exclusive ? kValidateErrorExclusiveMaximum : kValidateErrorMaximum, ValueType(actual).Move(), expected,
            exclusive ? &SchemaType::GetExclusiveMaximumString : RAPIDJSON_NULLPTR);
    }
    ...
    void BelowMinimum(int64_t actual, const SValue& expected, bool exclusive) {
        AddNumberError(exclusive ? kValidateErrorExclusiveMinimum : kValidateErrorMinimum, ValueType(actual).Move(), expected,
            exclusive ? &SchemaType::GetExclusiveMinimumString : RAPIDJSON_NULLPTR);
    }
...
private:
    ...
    void AddNumberError(const ValidateErrorCode code, ValueType& actual, const SValue& expected,
        const typename SchemaType::ValueType& (*exclusive)() = RAPIDJSON_NULLPTR) {
        currentError_.SetObject();
    ...
    */
    ...
    bool operator()(Handler& handler) {
        ...
        if (isValid_) {
            ...
            invalidSchemaKeyword_ = RAPIDJSON_NULLPTR;
