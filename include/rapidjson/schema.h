...
#define RAPIDJSON_SCHEMA_USE_PCREREGEX 0
#elif !RAPIDJSON_SCHEMA_USE_INTERNALREGEX &&                                   \
    defined(RAPIDJSON_SCHEMA_USE_PCREREGEX)
#define RAPIDJSON_SCHEMA_USE_PCREREGEX 1
...
#include <regex>
#elif RAPIDJSON_SCHEMA_USE_PCREREGEX
#include <pcrecpp.h>
#endif
...
public:
  ...
  Schema(SchemaDocumentType *schemaDocument, const PointerType &p,
      ...
      : allocator_(allocator), uri_(schemaDocument->GetURI(), *allocator),
        pointer_(p), typeless_(schemaDocument->GetTypeless()), enum_(),
        enumCount_(), not_(), type_((1 << kTotalSchemaType) - 1), // typeless
...
private:
  ...
  RegexType *CreatePattern(const ValueType &value) {
    if (value.IsString()) {
      return new (allocator_->Malloc(sizeof(RegexType)))
          RegexType(value.GetString(), pcrecpp::UTF8());
    }
...
private:
  ...
  internal::Stack<StateAllocator> schemaStack_; //!< stack to store the current
                                                //!< path of schema
                                                //!< (BaseSchemaType *)
  internal::Stack<StateAllocator> documentStack_; //!< stack to store the
                                                  //!< current path of
                                                  //!< validating document (Ch)
