...
public:
    ...
    //! Constructors
    GenericUri(Allocator* allocator = RAPIDJSON_NULLPTR) : uri_(), base_(), scheme_(), auth_(), path_(), query_(), frag_(), allocator_(allocator), ownAllocator_() {
    }

    GenericUri(const Ch* uri, SizeType len, Allocator* allocator = RAPIDJSON_NULLPTR) : uri_(), base_(), scheme_(), auth_(), path_(), query_(), frag_(), allocator_(allocator), ownAllocator_() {
        Parse(uri, len);
    }

    GenericUri(const Ch* uri, Allocator* allocator = RAPIDJSON_NULLPTR) : uri_(), base_(), scheme_(), auth_(), path_(), query_(), frag_(), allocator_(allocator), ownAllocator_() {
        Parse(uri, internal::StrLen<Ch>(uri));
    }

    // Use with specializations of GenericValue
    template<typename T> GenericUri(const T& uri, Allocator* allocator = RAPIDJSON_NULLPTR) : uri_(), base_(), scheme_(), auth_(), path_(), query_(), frag_(), allocator_(allocator), ownAllocator_() {
        const Ch* u = uri.template Get<const Ch*>(); // TypeHelper from document.h
        Parse(u, internal::StrLen<Ch>(u));
    }

#if RAPIDJSON_HAS_STDSTRING
    GenericUri(const String& uri, Allocator* allocator = RAPIDJSON_NULLPTR) : uri_(), base_(), scheme_(), auth_(), path_(), query_(), frag_(), allocator_(allocator), ownAllocator_() {
    ...
    SizeType GetStringLength() const { return uri_ == RAPIDJSON_NULLPTR ? 0 : internal::StrLen<Ch>(uri_); }
    const Ch* GetBaseString() const { return base_; }
    SizeType GetBaseStringLength() const { return base_ == RAPIDJSON_NULLPTR ? 0 : internal::StrLen<Ch>(base_); }
    const Ch* GetSchemeString() const { return scheme_; }
    SizeType GetSchemeStringLength() const { return scheme_ == RAPIDJSON_NULLPTR ? 0 : internal::StrLen<Ch>(scheme_); }
    const Ch* GetAuthString() const { return auth_; }
    SizeType GetAuthStringLength() const { return auth_ == RAPIDJSON_NULLPTR ? 0 : internal::StrLen<Ch>(auth_); }
    const Ch* GetPathString() const { return path_; }
    SizeType GetPathStringLength() const { return path_ == RAPIDJSON_NULLPTR ? 0 : internal::StrLen<Ch>(path_); }
    const Ch* GetQueryString() const { return query_; }
    SizeType GetQueryStringLength() const { return query_ == RAPIDJSON_NULLPTR ? 0 : internal::StrLen<Ch>(query_); }
    const Ch* GetFragString() const { return frag_; }
    SizeType GetFragStringLength() const { return frag_ == RAPIDJSON_NULLPTR ? 0 : internal::StrLen<Ch>(frag_); }
    ...
    bool Match(const GenericUri& uri, bool full = true) const {
        ...
        if (s1 == RAPIDJSON_NULLPTR || s2 == RAPIDJSON_NULLPTR) return false;
    ...
    GenericUri Resolve(const GenericUri& baseuri, Allocator* allocator = RAPIDJSON_NULLPTR) {
        GenericUri resuri;
...
private:
    ...
    Ch* CopyPart(Ch* to, Ch* from, std::size_t len) {
        RAPIDJSON_ASSERT(to != RAPIDJSON_NULLPTR);
        RAPIDJSON_ASSERT(from != RAPIDJSON_NULLPTR);
